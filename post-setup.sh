...
# usr/share ----------------------------------------------------
$ECHO "==> Copying wallpapers, sddm themes to /usr/share/"
...
function chaotic_add () {
    $ECHO "${GREEN}==> Enabling ParallelDownloads to 8${NC}"
...
function create_plymouth () {
    ...
    sed -i 's/ShowDelay=0/ShowDelay=0/' /etc/plymouth/plymouthd.conf
    sed -i 's/DeviceTimeout=8/DeviceTimeout=5/' /etc/plymouth/plymouthd.conf
    mkinitcpio -p linux-lts
}
...
$ECHO "${GREEN}
###############################################################################
# Done with Installation - Please Eject Install Media and Reboot
###############################################################################
${NC}"
